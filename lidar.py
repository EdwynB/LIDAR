...
class ExpressPacket(namedtuple('express_packet', 'distance angle new_scan start_angle')):
    ...
    def decode(cls, data):
        ...
        start_angle = (packet[2] + ((packet[3] & 0b01111111) << 8)) / 64 #Angle de départ du paquet ( / 2^6 == >> 6)
        ...
        for i in range(0, 80, 5): #on explore le paquet et on décode les mesures d'angles et de distances
            ...
            a += ((((packet[i + 8] & 0b00001111)) >> 3) * cls.sign[(packet[i + 4] & 0b00000001)],)  #fixed-point-q3 format.
            d += ((packet[i + 6] >> 1) + (packet[i + 7] << 7),)
            a += (((packet[i + 8] >> 4) >> 3) * cls.sign[(packet[i + 6] & 0b00000001)],)
